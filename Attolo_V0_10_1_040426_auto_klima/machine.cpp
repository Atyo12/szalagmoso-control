...
// ------------------------------------------------------------
struct VentAutoCmd {
  bool  anySensorOk = false;

  float rhMain      = 0.0f;  // TRH2 fő pára
  float tempHot     = 0.0f;  // TRH3 fő hővédelem
  float tempAux     = 0.0f;  // TRH1 kiegészítő

  float humDemand   = 0.0f;  // 0..1
  float tempDemand  = 0.0f;  // 0..1
  float mainDemand  = 0.0f;  // 0..1
...
static float mapDemandLinear(float x, float x0, float x1)
{
  if (x <= x0) return 0.0f;
  if (x >= x1) return 1.0f;
  return clamp01f((x - x0) / (x1 - x0));
}

static float max2f(float a, float b)
{
  return (a > b) ? a : b;
}

static void updateVentAutoDemand()
{
  const TrhState& trh1 = trhGet(TrhId::TRH1);
  const TrhState& trh2 = trhGet(TrhId::TRH2);
  const TrhState& trh3 = trhGet(TrhId::TRH3);

  bool trh1_ok = trhIsFresh(TrhId::TRH1);
  bool trh2_ok = trhIsFresh(TrhId::TRH2);
  bool trh3_ok = trhIsFresh(TrhId::TRH3);

  g_ventAuto = VentAutoCmd{};
  g_ventAuto.anySensorOk = trh1_ok || trh2_ok || trh3_ok;

  // Szerepek:
  // TRH2 = fő pára
  // TRH3 = fő hővédelem
  // TRH1 = kiegészítő hő referencia

  float rhMain  = trh2_ok ? trh2.humRH  : (trh1_ok ? trh1.humRH  : 55.0f);
  float tempHot = trh3_ok ? trh3.tempC  : (trh1_ok ? trh1.tempC  : 28.0f);
  float tempAux = trh1_ok ? trh1.tempC  : tempHot;

  g_ventAuto.rhMain  = rhMain;
  g_ventAuto.tempHot = tempHot;
  g_ventAuto.tempAux = tempAux;

  // Pára igény: 55%..85%
  float humDemand = mapDemandLinear(rhMain, 55.0f, 85.0f);

  // Hő igény: 28..40 °C
  float tempDemandMain = mapDemandLinear(tempHot, 28.0f, 40.0f);

  // Kiegészítő hő ág TRH1-ből, gyengébben
  float tempDemandAux = mapDemandLinear(tempAux, 30.0f, 42.0f) * 0.5f;

  float tempDemand = max2f(tempDemandMain, tempDemandAux);
  float mainDemand = max2f(humDemand, tempDemand);

  g_ventAuto.humDemand  = humDemand;
  g_ventAuto.tempDemand = tempDemand;
  g_ventAuto.mainDemand = mainDemand;

  const float FAN2_MIN_HZ = 19.0f;
  const float FAN2_MAX_HZ = 45.0f;
  const float FAN1_MIN_HZ = 10.0f;
  const float FAN1_MAX_HZ = 42.0f;

  // V1 stratégia: AUTO alatt legyen alapszellőzés
  float fan2Hz = FAN2_MIN_HZ + mainDemand * (FAN2_MAX_HZ - FAN2_MIN_HZ);

  float offset = (tempHot > 30.0f) ? 4.0f : 2.0f;
  float fan1Hz = fan2Hz - offset;

  fan1Hz = clampf(fan1Hz, FAN1_MIN_HZ, FAN1_MAX_HZ);
  fan2Hz = clampf(fan2Hz, FAN2_MIN_HZ, FAN2_MAX_HZ);
  ...
  // Ha egyetlen szenzor sem friss, menjen konzervatív fallback
  if (!g_ventAuto.anySensorOk) {
    fan2Hz = 25.0f;
    fan1Hz = 21.0f;
  }

  g_ventAuto.fan2Hz = fan2Hz;
  g_ventAuto.fan1Hz = fan1Hz;

  g_ventAuto.fan2Run   = true;
  g_ventAuto.fan1Run   = true;
  g_ventAuto.damperPct = 100.0f;

  g_fan1_should_run = g_ventAuto.fan1Run;
}
...
static void superviseFan1Damper()
{
  ...
  // AUTO / EREDETI LOGIKA
  // ============================================================
    // AUTO-ban a damper ne a tényleges futásból,
  // hanem az AUTO igényből döntsön
  // g_fan1_should_run értékét az updateVentAutoDemand() adja
  ...
  if (g_fan1_should_run) {
    ...
    if (!fan1DamperOpenFb()) {
      if (!t_fan1_damper_open_timeout.running) {
        t_fan1_damper_open_timeout.start(g_now, 20000);
      }
    ...
    if (!fan1DamperClosedFb()) {
      if (!t_fan1_damper_close_timeout.running) {
        t_fan1_damper_close_timeout.start(g_now, 20000);
      }
...
static void applyOutputs() {
  ...
  if (hmiGetVentManualMode()) {
    ...
    // AUTO mód
    // Fan1 csak akkor indulhat, ha a damper már tényleg elérte az indíthatósági küszöböt
    bool fan1RunFinal = g_ventAuto.fan1Run && g_fan1_run_permitted;

    vfdRequestFreqHz(VfdId::FAN1, g_ventAuto.fan1Hz);
    vfdRequestRun(VfdId::FAN1, fan1RunFinal);

    vfdRequestFreqHz(VfdId::FAN2, g_ventAuto.fan2Hz);
    vfdRequestRun(VfdId::FAN2, g_ventAuto.fan2Run);
  }
...
void machineLoop()
{
  ...
  // Raklap belépett -> sárga
  if (opto_rise) {
    g_semafor_green = false;
    g_semafor_yellow = true;
  }

  // Raklap elment az optó előtt -> indul az 5 impulzus számlálás
  if (opto_fall) {
    g_semafor_wait_encoder = true;
    g_encoder_target = g_encoder_count + SEMAFOR_PULSES;
  }

  // Megvan az 5 impulzus -> zöld
