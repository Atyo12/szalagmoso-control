...
// ------------------------------------------------------------
// FAN ramp (smooth control)
// ------------------------------------------------------------
static float g_fan2_cmd_hz = 0.0f;
static float g_fan1_cmd_hz = 0.0f;

// max változás / ciklus (~loop-onként)
static const float FAN_RAMP_UP_HZ   = 0.3f;
static const float FAN_RAMP_DOWN_HZ = 0.2f;

// base speed mosó alatt
static const float FAN2_BASE_HZ = 23.0f;
...
static float rampTo(float current, float target, float up_step, float down_step)
{
  if (target > current) {
    current += up_step;
...
static Timer t_fan2_stop_delay;
static constexpr uint32_t FAN2_STOP_DELAY_MS = 3000UL;
static Timer t_fan2_run_hold;
static bool  g_fan2_run_latched = false;
static constexpr uint32_t FAN2_RUN_HOLD_MS = 3000UL;
static constexpr float FAN2_START_HZ = 10.0f;
static constexpr float FAN2_STOP_HZ  = 1.0f;
...
static void applyAutoClimate()
{
  ...
  // Az enable bemenetet itt már nem használjuk tiltásra.
  // Meghívjuk, hogy ha később kell debughoz, megmaradjon a belső logika.
  (void)climateEnableEffective();
  ...
  // simított parancs
  g_fan2_command_hz = rampTo(g_fan2_command_hz, fan2_hz, FAN_RAMP_UP_HZ, FAN_RAMP_DOWN_HZ);
  g_fan1_command_hz = rampTo(g_fan1_command_hz, fan1_hz, FAN_RAMP_UP_HZ, FAN_RAMP_DOWN_HZ);

    // futási igény a simított értékből menjen
  g_fan1_should_run = (g_fan1_command_hz > 0.1f);

  float fan2_out_hz = g_fan2_command_hz;

  // FAN2 külön start/stop latch:
  // - indulás csak akkor, ha már értelmes frekiig felrámpázott
  // - leállás csak akkor, ha tényleg egy ideje nulla közelében van
  if (!g_fan2_run_latched) {
    if (g_fan2_command_hz >= FAN2_START_HZ) {
      g_fan2_run_latched = true;
      t_fan2_run_hold.start(g_now, FAN2_RUN_HOLD_MS);
    }
  } else {
    if (g_fan2_command_hz > FAN2_STOP_HZ) {
      t_fan2_run_hold.start(g_now, FAN2_RUN_HOLD_MS);
    } else {
      if (t_fan2_run_hold.running && t_fan2_run_hold.expired(g_now)) {
        g_fan2_run_latched = false;
        t_fan2_run_hold.stop();
      }
    }
  }

  bool fan2_out_run = g_fan2_run_latched;

  // ha futnia kell, ne küldjünk neki túl kicsi frekit
  if (fan2_out_run && fan2_out_hz < FAN2_START_HZ) {
    fan2_out_hz = FAN2_START_HZ;
  }

  // ha nem fut, akkor mehet 0 Hz
  if (!fan2_out_run) {
    fan2_out_hz = 0.0f;
  }

  vfdRequestFreqHz(VfdId::FAN2, fan2_out_hz);
  vfdRequestFreqHz(VfdId::FAN1, g_fan1_command_hz);

  vfdRequestRun(VfdId::FAN2, fan2_out_run);
  vfdRequestRun(VfdId::FAN1, (g_fan1_command_hz > 0.1f) && g_fan1_run_permitted);
}
...
// Wash stop retesz: ha a wash futása közben az OUT optó
// túl sokáig folyamatosan foglalt marad, a wash leáll,
// és csak OUT fall / force_empty után enged vissza.
static bool g_wash_hold_out_block = false;
...
static float g_ret_conv_speed_hz = 40.0f;
...
static void resetReturnState() {
  ...
  // OUT opto block watchdog nullázása is
  t_out_block.stop();
  t_out_free.stop();
  last_t_block_ms = 0;
  last_t_free_ms  = 0;
  g_wash_hold_out_block = false;
}
...
static void updateCounters() {
  ...
  if (di_ret_end.rise()) {
    // Védelem: csak akkor számoljuk ki a kilépő raklapot,
    // ha a return oldalon ténylegesen van bent számolt raklap.
    if (g_ret_out_count < g_ret_in_count) {
      g_ret_out_count++;
    } else {
      // Hibás / kézi szenzortrigger esetén ne menjen mínuszba a ret_load.
      g_ret_out_count = g_ret_in_count;
    }
  }
...
{
  // Ha E501 utáni HOME-recovery fut, az OUT optó ne reteszeljen vissza
  ...
  // ------------------------------------------------------------
  // 1) Wash stop csak akkor aktiválódjon,
  //    ha push ciklus közben érkezik egy ÚJ raklap:
  //    OUT optó FELFUTÓ él + push_active + nem HOME
  // ------------------------------------------------------------
  if (push_active &&
      !di_push_home.stable &&
      di_out_opto.rise()) {
  ...
  // ------------------------------------------------------------
  // 2) OUT-opto watchdog a wash szalagra:
  //    ha a wash ténylegesen fut és az OUT optó
  //    túl sokáig folyamatosan foglalt, akkor a wash álljon le.
  //
  //    A 322-es HMI paraméterhez kötött block_max_ms-t használjuk.
  //    Feloldás: OUT optó lefutó él vagy force_empty/resetReturnState().
  // ------------------------------------------------------------
  bool wash_running_now =
      !cmd_stop_req &&
      !g_wash_hold_until_home &&
      !g_wash_hold_out_block;

  if (wash_running_now && di_out_opto.stable) {
    if (!t_out_block.running) {
      t_out_block.start(g_now, g_params.block_max_ms);
    }

    last_t_block_ms = t_out_block.elapsed(g_now);

    if (t_out_block.expired(g_now)) {
      g_wash_hold_out_block = true;
      t_out_block.stop();
      last_t_block_ms = g_params.block_max_ms;
    }
  } else {
    t_out_block.stop();
    last_t_block_ms = 0;
  ...
  // OUT optó felszabadult -> watchdog retesz old
  if (di_out_opto.fall()) {
    g_wash_hold_out_block = false;
    t_out_block.stop();
    last_t_block_ms = 0;
  ...
  // ------------------------------------------------------------
  // 3) E501 számláló csak akkor induljon,
  //    ha OUT optó foglalt,
  //    az EXT már megvolt,
  //    és az áttoló már lejött az EXT-ről.
  // ------------------------------------------------------------
  if (di_out_opto.stable &&
      push_ext_seen &&
      !di_push_ext.stable &&
      !di_push_home.stable) {
    if (!t_danger.running) {
      t_danger.start(g_now, g_params.danger_time_ms);
    }
  }
...
static void supervisePushCycle() {
  if (!push_active) {
    Serial.print("[SPC EXIT] push_active=0 fault=");
    Serial.print((int)g_fault);
    Serial.print(" rec=");
    Serial.println(g_e305_recover_active);
    return;
  }
...
Serial.print("[REC END] now=");
Serial.print(g_now);
Serial.print(" fault=");
Serial.print((int)g_fault);
Serial.print(" rec=");
Serial.print(g_e305_recover_active);
Serial.print(" push_active=");
Serial.print(push_active);
Serial.print(" state=");
...
Serial.print(" ext=");
Serial.print(di_push_ext.stable);
...
static void applyOutputs() {
  bool stop_req = cmd_stop_req || g_fault_latched || g_wash_hold_until_home || g_wash_hold_out_block || !estopOk();
  ...
  bool green_light_cmd = reset_required ? g_reset_lamp_blink_state : true;
  bool alarm_lamp_cmd  = g_fault_latched;

  safeWrite(PIN_DO_GREEN_LIGHT, green_light_cmd);
  safeWrite(PIN_DO_JOG_LIGHT,   green_light_cmd);
  safeWrite(PIN_DO_ALARM_LAMP,  alarm_lamp_cmd);
  ...
  if (hmiGetVentManualMode()) {
    ...
    if (fan1Hz > 80.0f) fan1Hz = 80.0f;
    if (fan2Hz > 80.0f) fan2Hz = 80.0f;
  ...
  // LOCAL_JOG fault alatt is csak akkor mehessen,
  // ha a felső logika tényleg kiadta a parancsot
  // (tehát jog_btn + irány ténylegesen megvan).
  bool push_fwd =
    !cmd_push_rev &&
    estopOk() &&
    (
      ...
      (allow_fault_local_jog && cmd_push_motor)
    );
  ...
  bool push_rev =
    ...
    (
      (!g_fault_latched) ||
      (allow_fault_local_jog && cmd_push_rev)
    );
  ...
  // ------------------------------------------------------------
  // Szemafor kimenetek: a HMI alatti lámpákat tükrözik
  // ------------------------------------------------------------
    // ------------------------------------------------------------
  // Szemafor kimenetek: a HMI alatti lámpákat tükrözik
  // A sárga lámpa csak korlátozott ideig világítson fault esetén
  // (HMI 332 / g_params.buzzer_max_ms ideig)
  // ------------------------------------------------------------
  static bool semafor_yellow_last_req = false;
  static uint32_t semafor_yellow_start_ms = 0;

  bool semafor_yellow_req = alarm_lamp_cmd;

  if (semafor_yellow_req && !semafor_yellow_last_req) {
    semafor_yellow_start_ms = g_now;
  }
  if (!semafor_yellow_req) {
    semafor_yellow_start_ms = 0;
  }

  bool semafor_yellow_allowed = false;
  if (semafor_yellow_req) {
    if (semafor_yellow_start_ms == 0) semafor_yellow_start_ms = g_now;
    semafor_yellow_allowed =
      (uint32_t)(g_now - semafor_yellow_start_ms) < g_params.buzzer_max_ms;
  }

  semafor_yellow_last_req = semafor_yellow_req;

  safeWrite(PIN_DO_SEMAFOR_GREEN,  green_light_cmd);
  safeWrite(PIN_DO_SEMAFOR_YELLOW, semafor_yellow_allowed);
...
void machineSetupDefaults() {
  ...
  g_ret_conv_speed_hz = 40.0f;   // <-- EZ LEGYEN 40
