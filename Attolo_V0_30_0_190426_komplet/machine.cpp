...
static void supervisePushCycle() {
  ...
  if (g_push_phase == PushPhase::OUT) {
    if (di_push_ext.rise()) {
      ...
      // EXT elérve: nem lépünk még rögtön WAIT_TAIL_CLEAR-be,
      // előbb indul az átbukási holtidő
      cmd_push_motor = false;
      t_ext_tail_delay.start(g_now, EXT_TAIL_DELAY_MS);
    }
    ...
    if (push_ext_seen) {
      ...
      // Holtidő lejárt -> most vizsgáljuk a tail zónát
