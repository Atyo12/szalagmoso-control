...
static Timer t_ret_feed_pending_timeout;
static constexpr uint32_t RET_FEED_PENDING_MAX_MS = 3000;
...
static void updateCounters() {
  ...
  if (di_ret_start.rise()) {
    g_ret_in_count++;
    g_ret_feed_pending = false;
    t_ret_feed_pending_timeout.stop();
  }
...
static void superviseReturnFeedPending()
{
  if (!g_ret_feed_pending) {
    t_ret_feed_pending_timeout.stop();
    return;
  }

  // Ha már ténylegesen láttunk return oldali életjelet, a pending úgyis törlődik máshol.
  // Itt csak azt kezeljük, amikor hamis áttolás történt.
  bool no_return_evidence =
      !di_ret_start.stable &&
      !di_ret_end.stable &&
      (ret_load() <= 0);

  if (t_ret_feed_pending_timeout.running &&
      t_ret_feed_pending_timeout.expired(g_now) &&
      no_return_evidence) {
    g_ret_feed_pending = false;
    t_ret_feed_pending_timeout.stop();
  }
...
static void superviseReturnGhostLoad() {
  ...
  // Ha a returnnek épp mozognia kell vagy egy friss áttolás még folyamatban van,
  // akkor ne tekintsük ghost állapotnak azt, hogy a raklap a két szenzor között van.
  bool return_expected_to_move =
      cmd_ret_conv ||
      hmi_ret_conv_continuous ||
      g_ret_feed_pending;

  if (ret_load() > 0 && no_physical_pallet && !return_expected_to_move) {
...
static void supervisePushCycle() {
  ...
  t_ret_feed_pending_timeout.start(g_now, RET_FEED_PENDING_MAX_MS);

  if (!hmi_ret_conv_continuous) {
    cmd_ret_conv = true;
  }

  // EXT-re NEM állunk meg.
  // Csak elindítjuk az EXT utáni holtidőt.
  t_ext_tail_delay.start(g_now, g_params.danger_margin_ms);
    ...
    // EXT előtt és a holtidő alatt a push maradjon aktív
    cmd_push_motor = true;

    // Amíg nem láttunk EXT-et, nincs további teendő
    if (!push_ext_seen) {
      // megyünk tovább kifelé
    } else {
      // Amíg a holtidő fut, a ret_tail_occ még nem állít meg
      if (t_ext_tail_delay.running && !t_ext_tail_delay.expired(g_now)) {
        return;
      }

      // A holtidő lejárt. Most már számít a tail jel.
      ...
      // Ha a holtidő végére nincs tail foglaltság, indulhat a visszaút
