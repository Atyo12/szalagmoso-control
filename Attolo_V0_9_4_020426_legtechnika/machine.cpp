...
// Ha az AI skála nálad nem 0..1023, hanem pl. 0..4095,
// akkor ezt az értéket írd át 4095-re.
static constexpr float FAN1_AI_RAW_MAX = 1023.0f;

// Ha az AO kimenet más skálát vár, itt lehet később igazítani.
static constexpr float FAN1_AO_RAW_MAX = 1023.0f;
...
{
#if SIM_MODE
  // SIM-ben egyszerűen kövesse a parancsot
  return g_fan1_damper_cmd_pct;
#else
  int raw = analogRead(PIN_AI_FAN1_DAMPER_FB);

  float pct = (raw * 100.0f) / FAN1_AI_RAW_MAX;
...
static void updateInputs() {
  ...
  di_reset_main.update(safeRead(PIN_DI_RESET_MAIN), g_now, DEBOUNCE_MS);
...
static void superviseFan1Damper()
{
  ...
  if (hmiGetVentManualMode()) {
    float manPct = hmiGetFan1DamperSetPct();

    // HMI-ből jövő 0..100% parancs közvetlenül kimegy
    writeFan1DamperCommandPct(manPct);

    // kijelzett / kimenő érték
    g_fan1_damper_cmd_pct = manPct;

    // kézi módban az indítás feltétele:
    // a visszajelzett nyitás legalább 50%
    g_fan1_run_permitted = (g_fan1_damper_fb_pct >= 50.0f);

    // kézi módban ne fusson az auto open/close timeout felügyelet
    t_fan1_damper_open_timeout.stop();
    t_fan1_damper_close_timeout.stop();

    // ezek a bitek HMI-re mennek; kézi módban a százalék az érdekes,
    // de megtartjuk őket következetes állapotnak
  ...
  g_fan1_should_run = fan1St.running;
...
static void applyOutputs() {
  ...
  if (hmiGetVentManualMode()) {
    ...
    vfdRequestRun(VfdId::FAN1, fan1Run);
