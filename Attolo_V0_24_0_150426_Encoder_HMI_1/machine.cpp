...
static void applyAutoClimate()
{
  ...
  constexpr float    CLIMATE_ON_DEMAND_PCT   = 8.0f;
  constexpr float    CLIMATE_OFF_DEMAND_PCT  = 3.0f;
  constexpr uint32_t CLIMATE_MIN_RUN_MS      = 60000;
  ...
  if (!g_climate_auto_run) {
    ...
    // amíg a minimum futási idő nem telt le, addig nem állhat le
  ...
  if (in.trh1_ok && in.trh1_tempC > 30.0f) {
  ...
  if (in.trh1_ok && in.trh1_tempC >= 42.0f) {
