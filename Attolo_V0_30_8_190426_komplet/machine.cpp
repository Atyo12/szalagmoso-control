...
static void setFault(Fault f) {
  ...
  // E305 speciális kezelés:
  // álljon meg ott, ahol van, és NE induljon el automatikus hazamenet.
  // Reset után menjen csak vissza HOME-ba a meglévő recovery logikával.
...
Serial.print("[EXT DECISION] now=");
Serial.print(g_now);
Serial.print(" ext_seen=");
Serial.print(push_ext_seen);
Serial.print(" delay_run=");
Serial.print(t_ext_tail_delay.running);
Serial.print(" delay_exp=");
Serial.print(t_ext_tail_delay.expired(g_now));
Serial.print(" tail_raw=");
Serial.print(di_ret_tail_occ.raw);
Serial.print(" tail_stable=");
Serial.print(di_ret_tail_occ.stable);
Serial.print(" phase=");
Serial.println((int)g_push_phase);
  ...
  Serial.print(t_tail_clear_wait.running);
  Serial.print(" timer_exp=");
  Serial.print(t_tail_clear_wait.expired(g_now));
  Serial.print(" fault=");
  Serial.println((int)g_fault);
...
Serial.print("[PUSH END] phase=");
Serial.print((int)g_push_phase);
Serial.print(" fault=");
Serial.print((int)g_fault);
Serial.print(" latched=");
Serial.print(g_fault_latched);
Serial.print(" cmd_push=");
...
static void runStateMachineAuto() {
  ...
  if (g_fault_latched) {
    ...
    // E305 speciális recovery: csak reset után, csak előre HOME-ig
      ...
      // E305-nél ha közben újra foglalt lett a tail zóna, álljunk meg
  ...
  // Continuous mód NEM önálló RUN parancs.
  // Csak azt jelenti, hogy ne a push-ciklus léptesse a returnt,
  // hanem a return a saját igénye szerint működjön.
  ...
  if ((uint32_t)(g_now - lastRetDbg) >= 500) {
    ...
    Serial.print(" normal_need=");
    Serial.print(normal_need_return_dbg);

    Serial.print(" force_empty=");
    Serial.print(force_empty_need_return_dbg);

    Serial.print(" need=");
    Serial.print(need_return_dbg);

    Serial.print(" cmd=");
    Serial.println(cmd_ret_conv);*/
  }
}
...
static void runModeOverlayOrAuto() {
  ...
  // Fault utáni kontrollált hazatérés HOME-ba
  if (g_fault_hold_until_home) {
    cmd_stop_req   = true;
    cmd_ret_conv   = false;
    cmd_buzzer     = false;
    cmd_push_motor = false;
    cmd_push_rev   = !di_push_home.stable;
  ...
  if (hmiGetVentManualMode()) {
    ...
    if (g_fault_hold_until_home && di_push_home.stable) {
      cmd_push_motor = false;
      cmd_push_rev   = false;
      g_fault_hold_until_home = false;
      t_fault_home_recover.stop();
...
Serial.print(push_fwd);
Serial.print(" push_rev=");
Serial.println(push_rev);
