...
// ============================================================
// 2) KommunikĂˇciĂłs paramĂ©terek
// ============================================================
...
static const uint32_t FORCE_REFRESH_MS = 3000;

// ============================================================
// 3) Frekvencia paramĂ©terek
// ============================================================
// Ennek egyeznie kell a V20 P2000 referenciafrekvenciĂˇval.
static const float VFD_REF_HZ = 80.0f;
static const float FREQ_MIN_HZ = 5.0f;
static const float FREQ_MAX_HZ = 80.0f;
...
struct VFDDrive {
  ...
  // utoljĂˇra kikĂĽldĂ¶tt Ăˇllapot
  bool  applied_run;
...
static VFDDrive g_vfds[3] = {
  { 10, "Return", 0, 0, 0, 0, 0, false, 20.0f, false, -1.0f, 0 },
  { 12, "Fan1",   0, 0, 0, 0, 0, false, 20.0f, false, -1.0f, 0 },
  { 11, "Fan2",   0, 0, 0, 0, 0, false, 20.0f, false, -1.0f, 0 },
};

// ============================================================
// 6) Scheduler Ăˇllapot
// ============================================================
...
static uint8_t  g_nextWriteSearchStart = 0;
...
// ElsĹ‘ kĂ¶rben egyszerĹ±sĂ­tett running stĂˇtusz.
// KĂ©sĹ‘bb cserĂ©lhetĹ‘ statusWord bitdekĂłdolĂˇsra.
...
static bool driveNeedsWrite(const VFDDrive& d) {
  ...
  uint32_t now = millis();
  if ((uint32_t)(now - d.last_write_ms) >= FORCE_REFRESH_MS) return true;

  return false;
}

static int findNextDriveNeedingWrite() {
  for (int ofs = 0; ofs < ACTIVE_VFD_COUNT; ++ofs) {
    int idx = (g_nextWriteSearchStart + ofs) % ACTIVE_VFD_COUNT;
    if (driveNeedsWrite(g_vfds[idx])) {
      g_nextWriteSearchStart = (uint8_t)((idx + 1) % ACTIVE_VFD_COUNT);
      return idx;
    }
  }
  return -1;
...
void vfdPrintAll() {
  for (int i = 0; i < ACTIVE_VFD_COUNT; ++i) {
    ...
    Serial.print(d.last_ok_ms);

    Serial.print(" | last_write_ms=");
    Serial.println(d.last_write_ms);
  }
