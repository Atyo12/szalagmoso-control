...
if (g_mode == OpMode::LOCAL_JOG && estopOk()) {
  ...
  return;
        ...
        // ------------------------------------------------------------
    // Fault alatti LOCAL_JOG recovery
    // E301 / hasonló áttoló fault esetén engedjük a kézi HOME-ra húzást
    // ------------------------------------------------------------
    if (g_mode == OpMode::LOCAL_JOG && estopOk()) {
      bool jog_enable = hmi_service_enable;
      bool jog_btn    = di_jog_btn.stable;
      ...
      // Fault alatt is csak kézi nyomva-tartott jog menjen
      if (jog_enable && jog_btn && valid_dir) {
        if (jog_fwd) {
          cmd_push_motor = true;   // üzemi irány
        }
        if (jog_rev) {
          cmd_push_rev = true;     // HOME felé vissza
        }
      }

      // Ha HOME-ba ért és ez egy "hazavihető" fault volt, álljon meg
      if (di_push_home.stable) {
        cmd_push_motor = false;
        cmd_push_rev   = false;
      }
