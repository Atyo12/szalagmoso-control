...
static uint16_t clampFloatX10ToHr(float v)
{
  int32_t x10 = (int32_t)(v * 10.0f + (v >= 0.0f ? 0.5f : -0.5f));
  if (x10 < 0) return 0;
  if (x10 > 65535) return 65535;
  return (uint16_t)x10;
}

static void writeActualParamsToStatusRegs(const MachineParams& p)
{
  // debounce marad ms-ben
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_DEBOUNCE_MS, clampU16ToHr(p.debounce_ms));

  // HMI-n sec-ben megjelenők
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_FREE_MIN_SEC,  clampU16ToHr(p.free_min_ms / 1000UL));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_BLOCK_MAX_SEC, clampU16ToHr(p.block_max_ms / 1000UL));
  ...
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_SEMAFOR_PULSES,
                                       clampU16ToHr(p.semafor_pulses));

  // --- Légtechnika AUTO paraméterek (ACT)
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_CLIMATE_ON_DEMAND_PCT_X10,
                                       clampU16ToHr(p.climate_on_demand_pct_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_CLIMATE_OFF_DEMAND_PCT_X10,
                                       clampU16ToHr(p.climate_off_demand_pct_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_CLIMATE_MIN_RUN_SEC,
                                       clampU16ToHr(p.climate_min_run_sec));

  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_FAN2_AUTO_MIN_HZ_X10,
                                       clampU16ToHr(p.fan2_auto_min_hz_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_FAN2_AUTO_MAX_HZ_X10,
                                       clampU16ToHr(p.fan2_auto_max_hz_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_FAN1_AUTO_MIN_HZ_X10,
                                       clampU16ToHr(p.fan1_auto_min_hz_x10));

  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_HOT_OFFSET_TEMP_C_X10,
                                       clampU16ToHr(p.hot_offset_temp_c_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_HOT_FORCE_TEMP_C_X10,
                                       clampU16ToHr(p.hot_force_temp_c_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_HOT_FORCE_FAN2_MIN_HZ_X10,
                                       clampU16ToHr(p.hot_force_fan2_min_hz_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_HOT_FORCE_FAN1_MIN_HZ_X10,
                                       clampU16ToHr(p.hot_force_fan1_min_hz_x10));
}
...
static void writeActualParamsBackToEditRegs(const MachineParams& p)
{
  ...
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_FREE_MIN_SEC,  clampU16ToHr(p.free_min_ms / 1000UL));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_BLOCK_MAX_SEC, clampU16ToHr(p.block_max_ms / 1000UL));
  ...
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_SEMAFOR_PULSES,
                                       clampU16ToHr(p.semafor_pulses));

  // --- Légtechnika AUTO paraméterek: edit mezők visszatöltése
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_CLIMATE_ON_DEMAND_PCT_X10,
                                       clampU16ToHr(p.climate_on_demand_pct_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_CLIMATE_OFF_DEMAND_PCT_X10,
                                       clampU16ToHr(p.climate_off_demand_pct_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_CLIMATE_MIN_RUN_SEC,
                                       clampU16ToHr(p.climate_min_run_sec));

  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_FAN2_AUTO_MIN_HZ_X10,
                                       clampU16ToHr(p.fan2_auto_min_hz_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_FAN2_AUTO_MAX_HZ_X10,
                                       clampU16ToHr(p.fan2_auto_max_hz_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_FAN1_AUTO_MIN_HZ_X10,
                                       clampU16ToHr(p.fan1_auto_min_hz_x10));

  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_HOT_OFFSET_TEMP_C_X10,
                                       clampU16ToHr(p.hot_offset_temp_c_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_HOT_FORCE_TEMP_C_X10,
                                       clampU16ToHr(p.hot_force_temp_c_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_HOT_FORCE_FAN2_MIN_HZ_X10,
                                       clampU16ToHr(p.hot_force_fan2_min_hz_x10));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_HOT_FORCE_FAN1_MIN_HZ_X10,
                                       clampU16ToHr(p.hot_force_fan1_min_hz_x10));
}
...
static void processMachineParamCommands()
{
  auto applyEditRegsToMachine = []() {
    ...
    p.free_min_ms  = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_FREE_MIN_SEC)  * 1000UL;
    p.block_max_ms = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_BLOCK_MAX_SEC) * 1000UL;
    ...
    p.semafor_pulses =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_SEMAFOR_PULSES);

    // --- Légtechnika AUTO paraméterek
    p.climate_on_demand_pct_x10 =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_CLIMATE_ON_DEMAND_PCT_X10);

    p.climate_off_demand_pct_x10 =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_CLIMATE_OFF_DEMAND_PCT_X10);

    p.climate_min_run_sec =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_CLIMATE_MIN_RUN_SEC);

    p.fan2_auto_min_hz_x10 =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_FAN2_AUTO_MIN_HZ_X10);

    p.fan2_auto_max_hz_x10 =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_FAN2_AUTO_MAX_HZ_X10);

    p.fan1_auto_min_hz_x10 =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_FAN1_AUTO_MIN_HZ_X10);

    p.hot_offset_temp_c_x10 =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_HOT_OFFSET_TEMP_C_X10);

    p.hot_force_temp_c_x10 =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_HOT_FORCE_TEMP_C_X10);

    p.hot_force_fan2_min_hz_x10 =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_HOT_FORCE_FAN2_MIN_HZ_X10);

    p.hot_force_fan1_min_hz_x10 =
      (uint16_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_HOT_FORCE_FAN1_MIN_HZ_X10);
...
static void updateModbusOutputsFromStatus()
{
  ...
  modbusTCPServer.holdingRegisterWrite(
    HMI_HR_TRH1_TEMP_X10, clampX10(trh1.valid ? trh1.tempC : 0.0f));
  modbusTCPServer.holdingRegisterWrite(
    HMI_HR_TRH1_RH_X10, clampX10(trh1.valid ? trh1.humRH : 0.0f));

  modbusTCPServer.holdingRegisterWrite(
    HMI_HR_TRH2_TEMP_X10, clampX10(trh2.valid ? trh2.tempC : 0.0f));
  modbusTCPServer.holdingRegisterWrite(
    HMI_HR_TRH2_RH_X10, clampX10(trh2.valid ? trh2.humRH : 0.0f));

  modbusTCPServer.holdingRegisterWrite(
    HMI_HR_TRH3_TEMP_X10, clampX10(trh3.valid ? trh3.tempC : 0.0f));
  modbusTCPServer.holdingRegisterWrite(
    HMI_HR_TRH3_RH_X10, clampX10(trh3.valid ? trh3.humRH : 0.0f));
...
static void applyModbusInputsToHmi()
{
  ...
  if (speedX10 == 0) {
    // Ha a HMI mező még 0, ne nullázzuk le a PLC belső alapértékét.
    // Inkább töltsük vissza a PLC aktuális értékét a HMI-be.
    uint16_t plcSpeedX10 = (uint16_t)(hmiGetReturnConveyorSpeedHz() * 10.0f);
    modbusTCPServer.holdingRegisterWrite(HMI_HR_RET_SPEED_SET_X10, plcSpeedX10);
  } else {
    hmiSetReturnConveyorSpeedHz((float)speedX10 / 10.0f);
  }

  // Semafor pulses set
  uint16_t semaPulses = modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_SEMAFOR_PULSES);

  if (semaPulses == 0) {
    // Ha a HMI mező még 0 / üres, töltsük vissza a PLC aktuális értékét.
    uint16_t plcSemaPulses = (uint16_t)machineGetParams().semafor_pulses;
    modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_SEMAFOR_PULSES, plcSemaPulses);
  }
