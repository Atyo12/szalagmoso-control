...
static bool g_fan1_damper_should_open = false;
...
static bool  g_climate_wash_enable = false;
static Timer t_climate_min_run;
static Timer t_climate_wash_hold;
static DebouncedInput di_wash_running;
...
static void applyAutoClimate()
{
  ...
  // Javasolt: pára TRH2-ről, hő TRH1-ről
  ...
  uint32_t wash_hold_ms = (uint32_t)p.climate_wash_hold_sec * 1000UL;
bool wash_running = di_wash_running.stable;

bool wash_enable = false;

if (wash_running) {
  g_climate_wash_enable = true;
  t_climate_wash_hold.start(g_now, wash_hold_ms);
} else {
  if (t_climate_wash_hold.running && !t_climate_wash_hold.expired(g_now)) {
    g_climate_wash_enable = true;
  } else {
    g_climate_wash_enable = false;
    t_climate_wash_hold.stop();
  }
}

wash_enable = g_climate_wash_enable;
  bool hot_override = (in.trh1_ok && in.trh1_tempC >= hot_force_temp_c);
  bool humidity_override = (in.trh2_ok && in.trh2_rh >= 80.0f);

  bool climate_enable = wash_enable || hot_override || humidity_override;
  ...
  if (!climate_enable) {
    ...
    if (!g_climate_auto_run) {
      if (main_demand >= climate_on_demand_pct || hot_override || humidity_override) {
        g_climate_auto_run = true;
        t_climate_min_run.start(g_now, climate_min_run_ms);
      }
    } else {
      if (t_climate_min_run.expired(g_now)) {
        if (main_demand <= climate_off_demand_pct && !hot_override && !humidity_override) {
          g_climate_auto_run = false;
          t_climate_min_run.stop();
        }
      }
...
static MachineParams machineParamsDefaults()
{
  ...
  p.climate_wash_hold_sec = 1800;
  p.fan1_damper_hold_sec = 120;
...
static void clampMachineParams(MachineParams& p)
{
  ...
  p.climate_wash_hold_sec = (uint16_t)clampU32(p.climate_wash_hold_sec, 0, 14400);
  p.fan1_damper_hold_sec  = (uint16_t)clampU32(p.fan1_damper_hold_sec, 0, 1800);
...
static void updateInputs() {
  ...
  di_wash_running.update(safeRead(PIN_DI_WASH_RUNNING), g_now, g_params.debounce_ms);
}
...
{
  // ----------------------------------------------------------
  // 1) Raklap ráér az OUT optóra
  //    -> szabad idő vége
  //    -> rövid hézag ellenőrzése
  //    -> takarásidő indítása
  // ----------------------------------------------------------
  if (di_out_opto.rise()) {
    ...
    if (t_out_free.running) {
      ...
      // két takarás közötti szabad idő túl rövid
      if (last_t_free_ms > 0 && last_t_free_ms < g_params.free_min_ms) {
  ...
  // ----------------------------------------------------------
  // 2) Raklap lemegy az OUT optóról
  //    -> takarásidő vége
  //    -> túl hosszú takarás ellenőrzése
  //    -> szabad idő indítása
  // ----------------------------------------------------------
  if (di_out_opto.fall()) {
    ...
    if (t_out_block.running) {
      ...
      // túl hosszú takarás
      if (last_t_block_ms > g_params.block_max_ms) {
        setFault(Fault::E202_OUT_OPTO_STUCK_OR_DIRTY);
      }
    }

    // új szabad szakasz indul
    t_out_free.start(g_now, 0xFFFFFFFF);
  }

  // ----------------------------------------------------------
  // 3) Ha még mindig takarásban van és lejárt a max idő
  // ----------------------------------------------------------
...
static void superviseFan1Damper()
{
  ...
  // ============================================================
  uint32_t damper_hold_ms = (uint32_t)g_params.fan1_damper_hold_sec * 1000UL;

  if (g_fan1_should_run) {
    g_fan1_damper_should_open = true;
    t_fan1_damper_hold.start(g_now, damper_hold_ms);
  } else {
    if (t_fan1_damper_hold.running && !t_fan1_damper_hold.expired(g_now)) {
      g_fan1_damper_should_open = true;
    } else {
      g_fan1_damper_should_open = false;
      t_fan1_damper_hold.stop();
    }
  }

  if (g_fan1_damper_should_open) {
...
void machineSetupDefaults()
{
  ...
  // ------------------------------------------------------------
  // OUT OPTO időzítők indulási állapot
  // ------------------------------------------------------------
  last_t_free_ms = 0;
  last_t_block_ms = 0;

  bool out_opto = safeRead(PIN_DI_WASH_OUT_OPTO);

  // ha induláskor TAKART az opto -> blokkolt állapot
  if (out_opto) {
    t_out_block.start(g_now, g_params.block_max_ms);
  }
  // ha SZABAD -> szabad idő mérés indul
  else {
    t_out_free.start(g_now, 0xFFFFFFFF);
  }
