...
static void setFault(Fault f) {
  ...
  // E501 is ugyanígy: álljon meg faulttal,
  // és csak reset után menjen vissza HOME-ba
  if (f == Fault::E501_DANGER_HOME_LATE) {
    cmd_push_motor = false;
    cmd_push_rev   = false;
    g_fault_hold_until_home = false;
    t_fault_home_recover.stop();
...
static void runStateMachineAuto() {
  ...
  Serial.println(g_e305_recover_active);
    ...
    // E305 / E501 reset utáni recovery:
    // HOME nélkül ne teljes reset legyen, hanem kontrollált hazamenet.
    if (g_fault_latched &&
        !di_push_home.stable &&
        (
      ...
      g_fault_hold_until_home = false;   // ne a régi ág vigye el
      cmd_push_rev = false;
      t_fault_home_recover.start(g_now, g_params.push_cycle_max_ms);
      return;
    }
...
Serial.print(t_fault_home_recover.running && t_fault_home_recover.expired(g_now));
Serial.print(" cmd_before=");
Serial.println(cmd_push_motor);

      cmd_push_rev = false;   // biztosan ne ellenirány legyen

      // E305-nél ha újra foglalt a tail zóna, álljunk meg
      if (g_fault == Fault::E305_PUSH_TAIL_CLEAR_TIMEOUT &&
      ...
      // timeout védelem
      if (t_fault_home_recover.running &&
          t_fault_home_recover.expired(g_now)) {
        cmd_push_motor = false;
        cmd_push_rev   = false;
        g_e305_recover_active = false;
        return;
      ...
      // HOME felé reverse irány kell
      Serial.println("[REC] CMD_PUSH_REV=1");
      cmd_push_motor = true;
      return;
    }
