...
static DebouncedInput di_service_key;
static DebouncedInput di_local_jog;
...
static void updateInputs() {
  ...
  di_service_key.update(safeRead(PIN_DI_SERVICE_KEY), g_now, DEBOUNCE_MS);
  di_local_jog.update(safeRead(PIN_DI_LOCAL_JOG), g_now, DEBOUNCE_MS);
}
...
static void superviseReturnConveyorFaults() {
  if (di_ret_end.stable) {
    ...
    if (t_ret_end_soft.expired(g_now)) {
      cmd_stop_req = true;
    }
...
static void runStateMachineAuto() {
  ...
  if (!push_active) {
    bool need_return = (ret_load() > 0) || di_ret_start.stable || di_ret_end.stable;
...
if (hmi_ret_conv_continuous) {
  ret_run = !g_fault_latched && estopOk();
} else {
  ret_run = cmd_ret_conv && !g_fault_latched && estopOk();
}
  ...
  safeWrite(PIN_DO_PUSH_MOTOR, cmd_push_motor && estopOk());
...
if ((uint32_t)(g_now - lastPrint) >= 500) {
  lastPrint = g_now;

  VfdState retSt = vfdGetState(VfdId::RETURN);
  bool retRunReq = cmd_ret_conv && !g_fault_latched && estopOk();
