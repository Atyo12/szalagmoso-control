...
static void macroStart(SimMacro m)
{
  ...
  g_sim_di[PIN_DI_SERVICE_KEY]   = 0;
  g_sim_di[PIN_DI_LOCAL_JOG]     = 0;

  machineSetHmiHandleMode(false);
  machineSetHmiServiceEnable(false);
...
void simShow()
{
  ...
  Serial.print(" key="); Serial.print(g_sim_di[PIN_DI_SERVICE_KEY]);
  Serial.print(" jog="); Serial.print(g_sim_di[PIN_DI_LOCAL_JOG]);
  Serial.println();
}
...
static void simSerialParse()
{
  ...
  if (s == "cycle_ok") {
  macroStart(SimMacro::CYCLE_OK);
  return;
}

if (s == "fail_no_ext") {
  macroStart(SimMacro::FAIL_NO_EXT);
  return;
}

if (s == "fail_tail") {
  macroStart(SimMacro::FAIL_TAIL);
  return;
}

if (s == "fail_home") {
  macroStart(SimMacro::FAIL_HOME);
  return;
}
...
if (s == "macro_stop") {
  ...
  if (parseKV("key=", PIN_DI_SERVICE_KEY)) return;
  if (parseKV("jog=", PIN_DI_LOCAL_JOG)) return;
