...
static void writeActualParamsToStatusRegs(const MachineParams& p) {
  ...
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_PUSH_EXT_MAX_MS, clampU16ToHr(p.push_ext_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_PUSH_CYCLE_MAX_MS, clampU16ToHr(p.push_cycle_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_TAIL_CLEAR_MAX_MS, clampU16ToHr(p.tail_clear_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_RET_START_CLEAR_MAX_MS, clampU16ToHr(p.ret_start_clear_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_RET_END_SOFT_MS, clampU16ToHr(p.ret_end_soft_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_RET_END_HARD_MS, clampU16ToHr(p.ret_end_hard_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_DANGER_TIME_MS, clampU16ToHr(p.danger_time_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_DANGER_MARGIN_MS, clampU16ToHr(p.danger_margin_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_TABLE_WAIT_MAX_MS, clampU16ToHr(p.table_wait_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_ACT_BUZZER_MAX_MS, clampU16ToHr(p.buzzer_max_ms));
}
...
static void writeActualParamsBackToEditRegs(const MachineParams& p) {
  ...
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_PUSH_EXT_MAX_MS, clampU16ToHr(p.push_ext_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_PUSH_CYCLE_MAX_MS, clampU16ToHr(p.push_cycle_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_TAIL_CLEAR_MAX_MS, clampU16ToHr(p.tail_clear_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_RET_START_CLEAR_MAX_MS, clampU16ToHr(p.ret_start_clear_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_RET_END_SOFT_MS, clampU16ToHr(p.ret_end_soft_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_RET_END_HARD_MS, clampU16ToHr(p.ret_end_hard_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_DANGER_TIME_MS, clampU16ToHr(p.danger_time_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_DANGER_MARGIN_MS, clampU16ToHr(p.danger_margin_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_TABLE_WAIT_MAX_MS, clampU16ToHr(p.table_wait_max_ms));
  modbusTCPServer.holdingRegisterWrite(HMI_HR_PARAM_BUZZER_MAX_MS, clampU16ToHr(p.buzzer_max_ms));
}
...
static void processMachineParamCommands() {
  if (modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_APPLY_CMD) != 0) {
    ...
    p.push_ext_max_ms        = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_PUSH_EXT_MAX_MS);
    p.push_cycle_max_ms      = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_PUSH_CYCLE_MAX_MS);
    p.tail_clear_max_ms      = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_TAIL_CLEAR_MAX_MS);
    p.ret_start_clear_max_ms = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_RET_START_CLEAR_MAX_MS);
    p.ret_end_soft_ms        = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_RET_END_SOFT_MS);
    p.ret_end_hard_ms        = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_RET_END_HARD_MS);
    p.danger_time_ms         = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_DANGER_TIME_MS);
    p.danger_margin_ms       = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_DANGER_MARGIN_MS);
    p.table_wait_max_ms      = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_TABLE_WAIT_MAX_MS);
    p.buzzer_max_ms          = (uint32_t)modbusTCPServer.holdingRegisterRead(HMI_HR_PARAM_BUZZER_MAX_MS);
