...
{
  static uint32_t lastClimateDbgMs = 0;
  bool dbgPrint = false;

  if ((millis() - lastClimateDbgMs) >= 20000) {
    lastClimateDbgMs = millis();
    dbgPrint = true;
  }
  ...
  if (dbgPrint) {
    Serial.println("---- CLIMATE INPUT ----");
    Serial.print("override="); Serial.println(g_clima_test_override);

    Serial.print("TRH1 ok="); Serial.print(in.trh1_ok);
    Serial.print(" T="); Serial.print(in.trh1_tempC);
    Serial.print(" RH="); Serial.println(in.trh1_rh);

    Serial.print("TRH2 ok="); Serial.print(in.trh2_ok);
    Serial.print(" T="); Serial.print(in.trh2_tempC);
    Serial.print(" RH="); Serial.println(in.trh2_rh);

    Serial.print("TRH3 ok="); Serial.print(in.trh3_ok);
    Serial.print(" T="); Serial.print(in.trh3_tempC);
    Serial.print(" RH="); Serial.println(in.trh3_rh);
  }
  ...
  if (in.trh3_ok) temp_demand = temperatureDemandPct(in.trh3_tempC);

  float main_demand = (hum_demand > temp_demand) ? hum_demand : temp_demand;

  if (dbgPrint) {
    Serial.println("---- DEMAND ----");
    Serial.print("hum_demand="); Serial.println(hum_demand);
    Serial.print("temp_demand="); Serial.println(temp_demand);
    Serial.print("main_demand="); Serial.println(main_demand);
  }
  ...
  if (in.trh3_ok && in.trh3_tempC > 30.0f) {
  ...
  if (dbgPrint) {
    ...
    Serial.print("fan2_hz="); Serial.println(fan2_hz);
    Serial.print("fan1_hz="); Serial.println(fan1_hz);
  }
  ...
  bool fan2_run = (fan2_hz > 0.1f);
  bool fan1_run = (fan1_hz > 0.1f) && g_fan1_run_permitted;

  if (dbgPrint) {
    Serial.println("---- RUN REQUEST ----");
    Serial.print("fan2_run="); Serial.println(fan2_run);
    Serial.print("fan1_run="); Serial.println(fan1_run);
    Serial.print("fan1_permit="); Serial.println(g_fan1_run_permitted);
    Serial.println();
  }

  if (dbgPrint) {
  Serial.println("---- AUTO CLIMA ----");
  Serial.print("override="); Serial.println(g_clima_test_override);
  ...
  Serial.print("TRH3 ok="); Serial.print(in.trh3_ok);
  Serial.print(" T="); Serial.println(in.trh3_tempC);
  ...
  vfdRequestRun(VfdId::FAN2, fan2_run);
  vfdRequestRun(VfdId::FAN1, fan1_run);
}
